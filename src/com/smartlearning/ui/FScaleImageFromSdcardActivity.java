...
public class FScaleImageFromSdcardActivity extends Activity implements
		OnScrollListener{
	
	Bitmap bm;
	int IndexPposition = 1;
	int pageCount = 30;
	Context mContext = FScaleImageFromSdcardActivity.this;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.chat_select_image_from_sdcard);
		OperateSdcard();
	...
	public void init() {
		...
		imageAdapter = new ImageAdapter(FScaleImageFromSdcardActivity.this);
...
//						Toast.LENGTH_LONG).show();
	...
	public void btnClick(View v) {
		...
		case R.id.image_btn_back:
			FScaleImageFromSdcardActivity.this.finish();
		...
		case R.id.btn_preview:
			if(imageAdapter.ClickList.size()>0){
				Intent intent = new Intent(FScaleImageFromSdcardActivity.this,
						FPicturesShowActivity.class);
				Object[] data = imageAdapter.getClickList().toArray();
				intent.putExtra("selectPicture", data);
				FScaleImageFromSdcardActivity.this.startActivityForResult(intent,
				...
				Toast.makeText(FScaleImageFromSdcardActivity.this, "请选择图片", Toast.LENGTH_LONG).show();
			}
			break;
				...
				for (int i = 0; i < pageCount - 10; i++) {
					if (counter > (IndexPposition - 1) * pageCount + i) {
						...
						if (bm != null) {
							...
							getImgCounter = getImgCounter + 1;
							//Log.d("TAG", "bm" + bm);
							Bitmap btm2 = Bitmap.createScaledBitmap(bm, 200,
									200, false); // 这里你可以自定义它的大小
							bitmapList.add(btm2);
							myHandler.sendEmptyMessage(1);
						}
					}
